...
namespace Asiacell.ITADLibraries.LibSocket
{
    public class SocketServer
    {
        ...
        public SocketServer(int Port, SocketClientInfo SocketInfo, DataStoreCommand store, LoggerEntities logger, DBConnectionPool db)
        {
            this.logger = logger;
            this.db = db;
                ...
                if (bytesReader > 0)
                {
                    ...
                    if (content.IndexOf(SocketCommandConstant.EndCommand) > -1)
                    {
                        ...
                        if (SocketCommands.LoginCommand(content, ref CommandID, ref UserName, ref Password))
                        {
                            ...
                            ClientInfo useinfo = AllUserLogin.GetClientInfo(db.GetConnection(), UserName);

                            if (useinfo != null && useinfo.Password == Password)
                            {
                                ...
                                {
                                    ///Asign User properties to socket object
                                    loginObject.UserName = UserName;
                                    loginObject.Password = Password;
                                    loginObject.ConcrrentSession = useinfo.ConcurrentSession;
                                    loginObject.BufferSize = useinfo.BufferSize;
        ...
        private void ResponseResult(StateObject state, ref Task SendTask)
        {
            ...
            SendTask = Task.Factory.StartNew(delegate
            {
                ...
                while (!IsClosed)
                {
                    ...
                    {
                        PID = Functions.GetPID;
                        Send(senderState.ClientSocket, SocketCommands.GenerateResult(PID ,result.ErroCode, result.CommandID, SocketCommandConstant.CommandSubmited, state.UserName, result.Result));
                    }
                }
        ...
        private void ReadCallbackReceiver(IAsyncResult ar)
        {
            ...
            try
            {
                ...
                if (bytesRead > 0)
                {
                    ...
                    if (content.IndexOf(SocketCommandConstant.EndCommand) > -1)
                    {
                        ...
                        if (LogoutIndex > -1)
                        {
                            ...
                            {
                                logger.AddtoLog(PID,"Invalid command" + content, LoggerLevel.Info);
                                Send(state.ClientSocket, SocketCommands.GenerateResult(PID,SocketERRORCode.UnknowCommand, CommandID, SocketCommandConstant.LogoutCommand, state.UserName, "Invalid command"));
                            }
                        }
                        else
                ...
                {
                    SocketInfo.RemoveClient(state);
                    logger.AddtoLog(PID, "The connection is lost", LoggerLevel.Error);
                }
