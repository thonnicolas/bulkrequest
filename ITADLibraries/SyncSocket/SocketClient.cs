...
namespace Asiacell.ITADLibraries.Sockets
{
    public sealed class SocketClient : IDisposable
    {
        ...
        public string ReadToEndString(string EndString)
        {
            ...
            {
                result += "Socket read data error : " + IP + ", Erro Code:" + ex.SocketErrorCode + ", Description:" + ex.Message;
                throw new Exception(result);
            }
            catch (Exception ex)
            {
                result += "Socket read data error : " + IP + ", Erro Code:" + ex.Source + ", Description:" + ex.Message;
                throw new Exception(result);
            }
            return result;
        }
        ...
        /// <summary>
        /// Read data from buffer till "EndString". This function can be support with multi END STRING.
        /// Ex: Like Endstring1, Endstrign2
        /// </summary>
        /// <param name="EndString"></param>
        /// <returns></returns>
        public string ReadToEndString(string[] EndString)
        {
            ...
            try
            {
                ...
                do
                {
                    ...
                    result += Encoding.ASCII.GetString(bytesReceived, 0, b);
                    foreach (string ens in EndString)
                    {
                        if (result.IndexOf(ens) != -1)
                        {
                            isMatch = true;
                            break;//To exist from foreach loop.
                        } 
                    }
                    if (isMatch) break; //To exist from while loop.
                } while (b > 0);
            }
            catch (SocketException ex)
            {
