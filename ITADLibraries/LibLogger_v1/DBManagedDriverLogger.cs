...
{
    [Serializable]
    public class MyLogs1<T> : ICloneable where T:class
    {
    ...
    public class DBManagedDriverLogger<T> : ITaskExecutor, IDisposable where T : class
    {
        private int bulkBindCount;
        private DBManagedDriverDataTransaction bulkInsert;
        ...
        public DBManagedDriverLogger(DBManagedDriverConnection db, LoggerEntities logger, int bulkBindCount, string logFilePath, string logFile, int rotateSize, int recollectPeriod, IAttributeValidator attributeValidator, MissingTransactionLogger<T> tranLogger)
        {
        ...
        /// <param name="db"></param>
        /// <param name="bulkBindCount"></param>
        public DBManagedDriverLogger(DBManagedDriverConnection db, LoggerEntities logger, int bulkBindCount, string logFilePath, string logFile, int rotateSize, int recollectPeriod, string table_name, bool useBulkCopy, IAttributeValidator attributeValidator, MissingTransactionLogger<T> tranLogger)
        {
        ...
        /// <param name="db"></param>
        /// <param name="bulkBindCount"></param>
        public DBManagedDriverLogger(DBManagedDriverConnection db, LoggerEntities logger, int bulkBindCount, string logFilePath, string logFile, int rotateSize, int recollectPeriod, TaskExecutorManager taskManager, IAttributeValidator attributeValidator, MissingTransactionLogger<T> tranLogger)
        {
        ...
        private void Initialize(DBManagedDriverConnection db, LoggerEntities logger, int bulkBindCount, string logFilePath, string logFile, int rotateSize, int recollectPeriod,
            string table_name, bool useBulkCopy, TaskExecutorManager taskExectorManager, IAttributeValidator attributeValidator,   MissingTransactionLogger<T> tranLogger)
        {
            this.bulkBindCount = bulkBindCount;
            this.bulkInsert = new DBManagedDriverDataTransaction(table_name, db, bulkBindCount, useBulkCopy, logger);
        ...
        private void WriteLogToFile(IEnumerable<T> transactionObjects)
        {
            foreach (var l in transactionObjects)
            {
                ...
                {
                    logger.AddtoLog("Error add missing records with: ", ex , LoggerLevel.Info);
                }
            }
