...
using Asiacell.ITADLibraries.LibSocket;
...
{   
    ...
    public class LoggerEntities : IDisposable
    {
            ...
            String Command, DateTime Submited_On, DateTime Responded_On, LoggerLevel loglevel, Exception exception, ref StateObject state)
        {
            ...
            LoggerAttribute gendataLog = new LoggerAttribute(methodName, ProcessID, ERRORCODE, ClientIP, state.UserName, CommandID, Command, Submited_On, Responded_On, exception.ToString(), loglevel);
        ...
        public void AddtoLog(Exception ex, LoggerLevel logLevel)
        {
            ...
            logData.Enqueue(new LoggerAttribute(methodName, ex.ToString(), logLevel));
        }
        ...
        public void AddtoLog(Exception ex, LoggerLevel logLevel, ref StateObject state)
        {
            ...
            LoggerAttribute getlogdata = new LoggerAttribute(methodName, ex.ToString() , logLevel);
        ...
        public void AddtoLog(String PID, Exception ex, LoggerLevel logLevel, ref StateObject state)
        {
            ...
            LoggerAttribute getlogdata = new LoggerAttribute(PID, methodName, ex.ToString(), logLevel);
        ...
        public void AddtoLog(string PID, string Message,Exception ex,  LoggerLevel logLevel)
        {
            ...
            LoggerAttribute getlogdata = new LoggerAttribute(PID, methodName, Message + ", Error : " + ex.ToString(), logLevel);
            logData.Enqueue(getlogdata);
        ...
        {
            logger.Info("Staring Logger Entities Pool");
            try { 
            ...
            Addtask = Task.Factory.StartNew(() =>
            {
                while (true)
                {
                    try
                    {                        
                        if (token.IsCancellationRequested && logData.IsEmpty)
                            break;
                    ...
                    {
                        logger.Error(path + "\nAddToLogTask -> " + ex.ToString() );                        
                    }
                }
            }, token, TaskCreationOptions.LongRunning, TaskScheduler.Default);

            logger.Info("The logger entities pool collector is started...");
        }
        ...
        public String GenerateLogMessage(LoggerAttribute att)
        {
            ...
            {
                logger.Error("Error from GenerateLogMessage :" + ex.ToString());
                //throw;
            }
            return datalog;
        }
        ...
        public void Dispose()
        {
            try
            {
                logger.Info("Disposing Logger entities");
                if (tokenSource != null) tokenSource.Cancel();
                if (Addtask != null) Addtask.Wait();
                if (tokenSource != null) tokenSource.Dispose();
                logger.Info("The Logger entities is disposed");
                logger = null;
                System.Diagnostics.Process.GetCurrentProcess().WaitForExit(2000);
            }
            catch { }
